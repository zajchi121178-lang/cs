...
val top15Countries = df
  ...
  .filter($"population" > 0)
  .withColumn("percent_recovered", round(($"total_cases" / $"population") * 100, 2))
...
val task2 = df
  .filter($"date" >= "2021-03-25" && $"date" <= "2021-03-31" && $"new_cases".isNotNull && $"continent".isNotNull)
  .withColumn("rn", row_number().over(w2))
  .filter($"rn" === 1)
  .select($"date".alias("число"), $"location".alias("страна"), $"new_cases".alias("кол-во новых случаев"))
  .orderBy(desc("кол-во новых случаев"))
  .limit(10)

task2.show(false)
task2.coalesce(1).write.option("header", "true").mode("overwrite").csv("SparkSQL/result_task2")
...
val task3 = df
  .filter($"location" === "Russia" && $"date" >= "2021-03-24" && $"date" <= "2021-03-31" && $"new_cases".isNotNull)
  .select($"date".alias("число"), lag("new_cases", 1).over(w3).alias("кол-во новых случаев вчера"), $"new_cases".alias("кол-во новых случаев сегодня"))
  .filter($"кол-во новых случаев вчера".isNotNull && $"число" >= "2021-03-25")
  .withColumn("дельта", $"кол-во новых случаев сегодня" - $"кол-во новых случаев вчера")
  .select("число", "кол-во новых случаев вчера", "кол-во новых случаев сегодня", "дельта")
  .orderBy("число")

task3.show(false)
task3.coalesce(1).write.option("header", "true").mode("overwrite").csv("SparkSQL/result_task3")
